...
function checkProduct($data){
    ...
    $id = $data['product_id'];
    try{
        $response = $client->get('localhost/tdd-microservice-poc/index.php/api/1.0.0/product/'.$id);
...
function updateStock($amount, $id){
    ...
    try{
        $response = $client->put('localhost/tdd-microservice-poc/index.php/api/1.0.0/product/stock/'.$id, ['json' => $data]);
